 ...
 */
 ...
 #define PAGING_PAGE_PRESENT_FLAG 1 // Define the flag with an appropriate value
 #include <stdio.h>
 #include <pthread.h>
 ...
 int __free(struct pcb_t *caller, int vmaid, int rgid)
 {
   ...
   if(rgid < 0 || rgid > PAGING_MAX_SYMTBL_SZ)
     ...
     // Xóa thông tin vùng khỏi symbol table
 ...
 int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
 {
   ...
   { /* Page is not online, make it actively living */
     ...
     */
     ...
     MEMPHY_memcpy(caller->active_mswp, swpfpn, caller->mram, vicfpn);
 
     pte_set_swap(&mm->pgd[vicpgn], swpfpn, PAGING_PAGE_PRESENT_FLAG);
 
     MEMPHY_memcpy(caller->mram, vicfpn, caller->active_mswp, PAGING_PTE_SWP(pte));
 ...
 int get_free_vmrg_area(struct pcb_t *caller, int vmaid, int size, struct vm_rg_struct *newrg)
 {
   ...
   while (rgit != NULL)
   {
     ...
     if (free_size >= size)
     {
       // Cấp phát từ đầu vùng trống
       newrg->rg_start = rgit->rg_start;
       newrg->rg_end = rgit->rg_start + size;
 
       // Cập nhật lại vùng trống còn lại
       ...
       {
         // Vùng được cấp phát bằng toàn bộ vùng trống, loại bỏ khỏi danh sách
         struct vm_rg_struct *to_free = rgit;
         rgit = rgit->rg_next;
         cur_vma->vm_freerg_list = rgit;
         free(to_free);
       }
   return 0;
 }
